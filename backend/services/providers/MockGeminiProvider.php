...
final class MockGeminiProvider implements GeminiProvider
{
    ...
    public function generateCharacters(array $context): array
    {
        ...
        $snippet = substr(preg_replace('/\s+/', ' ', (string) ($context['book_text'] ?? '')), 0, 80);
    ...
    public function generatePortrait(array $context, array $character): array
    {
        ...
        $this->writePlaceholderImage($this->storageRoot . '/' . $relative);

        return [
            'relative_path' => $relative,
            'image_interaction_id' => "mock-image-{$projectId}-portrait-{$characterId}",
        ];
    }
    ...
    {
        $projectId = (int) $context['project_id'];
        $names = array_column($context['characters'] ?? [], 'name');
        $cast = $names !== [] ? implode(' and ', $names) : 'the characters';

        return [
            'name' => 'Riverbank Morning',
            'prompt' => "One key scene featuring {$cast} beside the river at dawn.",
            'text_interaction_id' => "mock-text-{$projectId}-chapters",
        ];
    }
    ...
    {
        $projectId = (int) $context['project_id'];
        $relative = "images/illustrations/{$projectId}/chapter-1.png";

        $this->writePlaceholderImage($this->storageRoot . '/' . $relative);

        return [
            'relative_path' => $relative,
            'image_interaction_id' => "mock-image-{$projectId}-illustration",
        ];
    }
    ...
    private function writePlaceholderImage(string $absolutePath): void
    {
        ...
        if ($png === false) {
            throw new RuntimeException('Invalid placeholder image data.');
        }

        if (file_put_contents($absolutePath, $png) === false) {
            throw new RuntimeException('Could not write placeholder image.');
        }
    }
}
